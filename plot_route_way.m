function [] = plot_route_way(ax, parsed_osm, route)
...
show_ways(ax, bounds, node, way, map_img_filename, route);

function [] = show_ways(hax, bounds, node, way, map_img_filename, route)
...
for i=1:size(way.id, 2)
    ...
    if ~isempty(nd_coor)
        ...
        if flag == 1
            plot(hax, nd_coor(1,:), nd_coor(2,:), 'ro-')
        else
